...
W_TEMP      EQU 0x70    ; Memoria compartida para interrupciones
STATUS_TEMP EQU 0x71
    
 CBLOCK 0x20
    ; Contadores
        MS_CONT_L      ;guardan milisegundos   
        MS_CONT_H           
        BEAT_CONT        ;contador de latidos

    ; Variables para displays
        UNIDADES         
        DECENAS          
        CENTENAS         
        DISP_ACTIVO; selecciona el dsiplay 
	
    ; Resto 
        AUX          
        ESTADO_PULSO   ;Controla si el latido actual ya fue contado =1 LATIDO YA DETECTADO
        BEEP_TIMER ;para que el BUZZER tenga duración determinada
        BPM    

    ; Variables UART y ADC continuos
        CMD ;Guarda el último carácter recibido por UART          
	FLAG_ADC;indicar que hay un dato ADC listo para enviar
        UART_TEMP ; Reemplaza W_TEMP en subrutinas para no corromper ISR
...
ISR:
    ...
    BANKSEL RCREG
    MOVF RCREG, W
    MOVWF CMD ; Guardar comando recibido (RCIF se limpia al leer)
...
CHK_ADC:
    ...
    GOTO END_ISR        
    BCF PIR1, ADIF      
; ENVÍO UART (Si el Flag está activo)
    BANKSEL FLAG_ADC
    BTFSS FLAG_ADC, 0
    GOTO ANALIZAR_PULSO

; Evitamos enviar datos cada 1ms, lo bajamos a 50ms para no saturar TX
...
ANALIZAR_PULSO:
; Evaluar si hay latido (ADC 8 bits, ADFM=0, Umbral 0x80 = 2.5V)
    BANKSEL ADRESH
    MOVF ADRESH, W
...
; UART Config
    ...
    BANKSEL TXSTA
    MOVLW b'00100100'     
    MOVWF TXSTA; habilita transmision y en alta velocidad
    
    BANKSEL RCSTA
    MOVLW b'10010000'   
    MOVWF RCSTA; habilita recepcion, que sea continuo 

; Timer0 Config
    BANKSEL OPTION_REG
    MOVLW b'11010001'; Prescaler 1:4 (1ms ticks) 255*4 
    MOVWF OPTION_REG 
...
LOOP:
; Verificación 10000ms (0x2710) =10 seg
    BANKSEL MS_CONT_H
    MOVF MS_CONT_H, W
    SUBLW 0x27
    BTFSS STATUS, Z
...
; Pasar a BCD para displays
    ...
    MOVF BPM, W         ; Cargar el valor numérico del BPM
    CALL UART_SendDecimal
    GOTO LOOP
    END